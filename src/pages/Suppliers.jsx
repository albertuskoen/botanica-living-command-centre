...
import { nextId } from '../utils/format.js'
    ...
    <div>
              ...
              <tbody>
                {suppliers.map(s => (
                  <tr key={s.id}>
                    ...
                    <td style={{ maxWidth:180, fontSize:12, color:T.textMid }}>{s.notes?.substring(0,55)}{s.notes?.length > 55 ? '…' : ''}</td>
                    <td>
                      <div style={{ display:'flex', gap:4 }}>
                        <button className="btn btn-outline btn-xs" onClick={() => openEdit(s)}>Edit</button>
                        <button className="btn btn-xs" style={{ background:'transparent', border:'none', cursor:'pointer', color:T.textLight }} onClick={() => del(s.id)}>✕</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      >
        <div className="form-grid">
          ...
          <div className="form-field"><label>Terms</label><select value={form.terms} onChange={F('terms')}><option>EXW</option><option>FOB</option><option>CIF</option></select></div>
          <div className="form-field"><label>Status</label><select value={form.status} onChange={F('status')}><option>Active</option><option>Inactive</option><option>Prospective</option></select></div>
